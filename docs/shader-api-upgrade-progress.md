...
- [ ] Matrix-based SceneView camera setter in addition to the existing position/target setter (in progress — `dxSetSceneViewMatrix` and `DxSceneView:setMatrix` validate finite, non-degenerate basis vectors, normalize the camera basis and reuse the existing internal `CMatrix` path; Debug Win32 build passes and runtime validation remains)
- [ ] Orthographic SceneView projection (planned — explicit perspective/orthographic mode, validated view volume/near/far parameters and capability-safe fallback; the current FOV path remains perspective and backward-compatible)
