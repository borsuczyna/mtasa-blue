...
- API: `dxApplyShaderToSceneViewWorldTexture(shader, sceneView, textureName)` and `dxRemoveShaderFromSceneViewWorldTexture(shader, sceneView, textureName)`. The first safe version supports wildcard texture matching across the view; target-element filtering can be added after the isolated matcher is proven. Both elements must belong to the calling resource.
