...
Raise the Stage-1 cap of 1 view/frame to a capability-reported maximum, still hard-enforced. Add `dxSetSceneViewUpdateMode(view, mode [, value])` with `manual` (the backward-compatible default used with `dxRequestSceneViewRender`), `once`, `always`, `every_n_frames`, and millisecond `interval` modes. Add per-view GPU-time accounting through asynchronous `IDirect3DQuery9` support. Resource authors select target dimensions and update frequency directly; the scheduler does not throttle by resolution or pixel count. Reuses Stage 1 item 13's exact per-view render procedure — this stage is scheduling and diagnostics around it, not a new rendering mechanism. Done when N views (N = capability max) render simultaneously with correct independent restoration, re-validated per item 14's test at N>1.
...
- Proposed API direction: `dxApplyShaderToSceneViewWorldTexture(shader, sceneView, textureName [, targetElement] [, appendLayers])` and `dxRemoveShaderFromSceneViewWorldTexture(shader, sceneView, textureName [, targetElement])`, following the argument order and matching rules of the existing `engineApplyShaderToWorldTexture` API. Exact naming remains subject to Lua convention review.
- **Output post-processing**: `dxGetSceneViewTexture(sceneView)` remains a side-effect-free texture getter. A shader can already sample that returned texture in an ordinary safe render pass; a later declarative convenience API may attach an ordered post-process pass chain to a view and expose its final output texture. It must ping-pong between distinct targets, detect read/write feedback, run after the world render, and never reinterpret a material shader as a fullscreen shader.
